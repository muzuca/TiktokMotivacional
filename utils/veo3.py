...
def _ffmpeg_or_die() -> str:
    ...
    if not p: raise RuntimeError("ffmpeg não encontrado no PATH.")
    return p

def _ffprobe_or_die() -> str:
    p = shutil.which("ffprobe")
    if not p: raise RuntimeError("ffprobe não encontrado no PATH.")
    return p
...
def _pick_bgm_path() -> str:
    if callable(_pick_bgm_util):
        ...
        except Exception as e: logger.debug("Falha ao escolher trilha via utils.audio: %s", e)
    base = "audios"
...
def _get_client() -> "genai.Client":
    if not _HAVE_GENAI: raise RuntimeError("Instale: pip install -U google-genai")
    return genai.Client()
...
def _ask_gemini_viral_analysis(tema: str, persona: str, idioma: str) -> Dict[str, str]:
    ...
    try:
        ...
        required_keys = ["viral_angle", "keywords", "tone_of_voice", "visual_style", "cinematography_suggestions", "character_action", "hook_style", "cta_style"]
        ...
        return {
            "viral_angle": f"A personal message about {tema}.", "keywords": tema, "tone_of_voice": "Confident and welcoming.",
            "visual_style": "Cinematic selfie.", "cinematography_suggestions": "Alternate between Medium Shot and Close-up.",
            "character_action": "Speak directly to the camera with a gentle smile.",
            "hook_style": "Gentle and inviting.", "cta_style": "Soft and engaging."
        }
...
def _build_gemini_command_full(persona: str, idioma: str, tema: str, n: int, variation_mode: str, viral_context: Dict[str, str]) -> str:
...
def _parse_prompts_from_gemini_json(raw_text: str, n: int) -> List[str]:
    ...
    except Exception as e: raise RuntimeError(f"Resposta do Gemini não é JSON válido: {e}")
    ...
    for s in scenes:
        ...
        if block: prompts.append(block)
    if prompts and len(prompts) != n:
        logger.warning("Gemini retornou um número de prompts diferente do esperado (%d/%d).", len(prompts), n)
    return prompts

def _ask_gemini_scene_prompts(persona: str, idioma: str, tema: str, n: int, variation_mode: str, viral_context: Dict[str, str]) -> List[str]:
    client = _get_client()
...
def _filter_for_input_nozoom(i: int) -> str:
    return (
        f"[{i}:v]scale={OUT_W}:{OUT_H}:force_original_aspect_ratio=decrease,pad={OUT_W}:{OUT_H}:(ow-iw)/2:(oh-ih)/2,fps={FPS_OUT},format=yuv420p,setsar=1/1[v{i}];[{i}:a]aformat=sample_fmts=s16:channel_layouts=stereo:sample_rates={AUDIO_SR}[a{i}];"
    )

def _stitch_and_bgm_ffmpeg(mp4s: List[str], out_path: str, bgm_path: str = "", bgm_db: float = BG_MIX_DB) -> str:
    if not mp4s: raise ValueError("Lista de vídeos vazia.")
    ffmpeg = _ffmpeg_or_die()
    cmd = [ffmpeg, "-y", "-loglevel", "error", "-stats"]
    for p in mp4s: cmd += ["-i", p]
    bgm_idx = -1
    if bgm_path and os.path.isfile(bgm_path):
        cmd += ["-i", bgm_path]
        bgm_idx = len(mp4s)
    parts = []
    for i in range(len(mp4s)): parts.append(f"[{i}:v]scale={OUT_W}:{OUT_H}:force_original_aspect_ratio=decrease,pad={OUT_W}:{OUT_H}:(ow-iw)/2:(oh-ih)/2,fps={FPS_OUT},format=yuv420p,setsar=1/1[v{i}];[{i}:a]aformat=sample_fmts=s16:channel_layouts=stereo:sample_rates={AUDIO_SR}[a{i}];")
    va = "".join([f"[v{i}][a{i}]" for i in range(len(mp4s))])
    parts.append(f"{va}concat=n={len(mp4s)}:v=1:a=1[vcat][acat];")
    if bgm_idx >= 0:
        parts.append(f"[{bgm_idx}:a]aformat=sample_fmts=s16:channel_layouts=stereo:sample_rates={AUDIO_SR},aloop=loop=-1:size=2e9,aresample={AUDIO_SR}[bg_looped];")
...
def _post_zoom_ffmpeg(src_path: str, dst_path: str, zoom: float) -> str:
    ...
    if zoom <= 1.0001:
        ...
        return dst_path
    filter_v = (f"[0:v]scale=iw*{zoom}:ih*{zoom},crop={OUT_W}:{OUT_H}:(iw-{OUT_W})/2:(ih-{OUT_H})/2,fps={FPS_OUT},format=yuv420p,setsar=1/1[v]")
    cmd = [
...
def _normalize_hashtags(hashtags, k: int = HASHTAGS_TOP_N) -> List[str]:
    ...
    def _push(h: str):
        h = (h or "").strip()
        if not h: return
        if not h.startswith("#"): h = "#" + re.sub(r"^\W+", "", h)
...
def _postar_video(final_video: str, idioma: str):
    ...
    except Exception as e: logger.exception("❌ Falha ao postar no TikTok: %s", e)
...
def _listar_slugs() -> Dict[str, Dict[str, List[str]]]:
    ...
    try:
        for name in os.listdir(VIDEOS_DIR):
            ...
            if idx: d["cenas"].append(path)
            elif "final" in name: d["final"].append(path)
            else: d["test"].append(path)
    except Exception as e: logger.debug("Falha ao listar vídeos: %s", e)
    for d in items.values():
        for k in d: d[k].sort(key=lambda p: os.path.getmtime(p), reverse=True)
    return items
...
def _menu_reutilizar_videos(idioma: str) -> None:
    ...
    ordered = sorted(slugs.items(), key=lambda kv: os.path.getmtime((kv[1]["final"] or kv[1]["cenas"] or kv[1]["test"])[0]), reverse=True)
    ...
    if d["cenas"]:
        ...
        print("\nComo deseja proceder?"); print("1. POSTAR um arquivo específico"); print("2. **EXPORTAR** um arquivo específico (sem postar)")
        modo = {"1": "2", "2": "4"}.get(input("Escolha 1 ou 2: ").strip(), "2")
    if modo in ("1", "3"):
        if not d["cenas"]: print("Não há cenas para juntar."); return
        ...
        try:
            ...
            logger.warning("Falha no stitch/BGM: %s", e); final_video = mp4s[0]
        if POST_ZOOM > 1.0001:
        ...
        if modo == "3": print(f"✅ Exportado (sem postar): {final_video}"); return
        _postar_video(final_video, idioma)
        return
    todos = d["final"] + d["test"] + d["cenas"]
    if not todos: print("Nenhum arquivo disponível."); return
    print("\nArquivos disponíveis:")
    for i, p in enumerate(todos, 1): print(f"{i}. {os.path.basename(p)}")
    op2 = input("Arquivo (número): ").strip()
    if not op2.isdigit() or not (1 <= int(op2) <= len(todos)): print("Cancelado."); return
    escolhido = todos[int(op2) - 1]
    if modo == "2": _postar_video(escolhido, idioma); return
    ...
    except Exception as e: logger.warning("Falha ao exportar: %s", e)

def _person_generation_for(idioma: str, persona: str) -> str:
    idioma, persona = (idioma or "").lower(), (persona or "").lower()
...
def _veo3_generate_single_api(prompt_text: str, out_path: str, idioma: str, persona: str) -> str:
    # ... (esta função está no arquivo do usuário, mas não é chamada, então a omiti por brevidade)
    pass
...
def _veo3_generate_batch(prompts: List[str], out_paths: List[str], idioma: str, persona: str) -> List[str]:
    ...
    if USE_BACKEND == "flow":
        ...
        if _HAVE_FLOW_MANY:
            ...
            try:
                return generate_many_via_flow(
                    prompts=prompts, out_paths=out_paths, project_url=FLOW_PROJECT_URL,
                    cookies_file=FLOW_COOKIES_FILE, headless=VEO3_HEADLESS,
                    timeout_sec=int(float(os.getenv('FLOW_TIMEOUT_SEC','600'))),
                )
            except Exception as e: logger.warning("generate_many_via_flow falhou (%s); farei single por cena.", e)
        ...
        for ptxt, outp in zip(prompts, out_paths):
                ...
                prompt_text=ptxt, out_path=outp, project_url=FLOW_PROJECT_URL,
                cookies_file=FLOW_COOKIES_FILE, headless=VEO3_HEADLESS,
                timeout_sec=int(float(os.getenv('FLOW_TIMEOUT_SEC','600'))),
            ))
        return done
    logger.info("🧠 API backend: gerando %d cena(s)…", len(prompts))
    # A chamada para a API single estava faltando, recuperando-a
...
def _selecionar_assunto(persona: str) -> str:
    ...
    for i, e in enumerate(exemplos, 1): print(f"{i}. {e}")
    print("0. Outro (digitar manualmente)")
    op = input("Opção: ").strip()
    if op == "0": return (input("Digite o tema (curto): ").strip() or "Prosperidade")
    if op.isdigit() and 1 <= int(op) <= len(exemplos): return exemplos[int(op) - 1]
    return "Prosperidade"

def _selecionar_qtd_cenas() -> int:
    print("\nQuantas cenas deseja gerar? (cada cena ≈ 8s)"); print("Sugerido: 2")
    raw = input("Digite 1–4 (Enter=2): ").strip()
    if raw == "": return 2
    try: n = int(raw); return max(1, min(4, n))
    except Exception: return 2

def _selecionar_variacao() -> str:
    print("\nVariação de cenário/roupa:")
    print("1. Manter cenários e roupas padrão"); print("2. Trocar cenário e manter roupa")
    print("3. Trocar roupa (cor/estilo) e manter cenário"); print("4. Trocar roupa e trocar cenário")
    print("5. Aleatório")
...
def _submenu_acao() -> str:
    ...
    print("1. Gerar APENAS para testes (não postar)"); print("2. Gerar e POSTAR")
    print("3. REUTILIZAR vídeo(s) existente(s)"); print("4. Gerar e **EXPORTAR** (sem postar)")
...
def executar_interativo(persona: str, idioma: str) -> None:
    ...
    while True:
        tema, n_cenas, variacao, escolha = _selecionar_assunto(persona), _selecionar_qtd_cenas(), _selecionar_variacao(), _submenu_acao()
        if escolha == "3": _menu_reutilizar_videos(idioma); return
        ...
        if not _preview_and_confirm_prompts(slug, prompts): print("❌ Prompts reprovados. Voltando ao menu."); continue
        ...
        if escolha == "1":
            print("\n✅ Cenas de teste geradas:", *[f"\n - {p}" for p in mp4s]); print("\nUse 'Reutilizar' para juntar/postar/exportar."); return
        trilha = _pick_bgm_path()
...
def postar_em_intervalo(persona: str, idioma: str, cada_horas: float) -> None:
    persona, idioma = (persona or "luisa").lower(), (idioma or "pt-br").lower()
    base_temas, intervalo = _sugerir_assuntos(persona), float(cada_horas) * 3600.0
    try:
        while True:
            inicio = datetime.now()
            ...
            try:
                viral_context = _ask_gemini_viral_analysis(tema, persona, idioma)
                prompts = _ask_gemini_scene_prompts(persona, idioma, tema, n=2, variation_mode="keep_all", viral_context=viral_context)
